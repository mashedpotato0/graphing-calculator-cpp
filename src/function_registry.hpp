...
public:
  ...
  void define(const std::string &name, const std::vector<std::string> &params,
              std::shared_ptr<expr> body, const std::string &src = "") {
  ...
  bool define_from_string(const std::string &def_str, const context &ctx,
    ...
    if (!ast) {
      error = "could not parse body: " + body_str;
      return false;
    }
  ...
  double call_numeric(const std::string &name, const std::vector<double> &args,
                      context &ctx) const {
    auto def = get(name);
    if (!def)
      throw std::runtime_error("Unknown function: " + name);
